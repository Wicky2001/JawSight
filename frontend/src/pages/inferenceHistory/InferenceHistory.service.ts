...
const placeholders: InferenceHistoryRowType[] = Array.from(
  { length: 100 },
  (_, index) => ({
    patient_id: index + 1,
    ...
    status:
      index % 3 === 0 ? "COMPLETED" : index % 3 === 1 ? "PROCESSING" : "FAILED",
...
export const fetchInferenceHistory = async (
  ...
  if (USE_PLACEHOLDERS) {
    ...
    if (params.sortField) {
      ...
      filtered.sort((a, b) => {
        const aValue = String(
          a[params.sortField as keyof InferenceHistoryRowType] ?? "",
        );
        const bValue = String(
          b[params.sortField as keyof InferenceHistoryRowType] ?? "",
        );
